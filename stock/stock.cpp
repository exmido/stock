...
int main()
{
	...
	nwt.connect(1, inner_size + 1, out_size, new nn::act_elu<double>(), new nn::opt_adam<double>());
	nwt.connect(0, in_size + 1, inner_size, new nn::act_tanh<double>(), new nn::opt_adam<double>());
	nwt.io_reset(1);
	...
	//target
	auto* target = nw.io_ptr(static_cast<int32_t>(nwt.io.size()), nwt.out_size(), 0);

	nw.connect(1, inner_size + 1, out_size, new nn::act_elu<double>(), new nn::opt_adam<double>());
	nw.connect(0, in_size + 1, inner_size, new nn::act_tanh<double>(), new nn::opt_adam<double>());
	nw.io_reset(1);
	...
	for (int32_t e = 0; e < epoch;)
	{
		for (auto i = window; i < r - test; ++i)
		{
			...
			if (0 == i % 10)
			{
				for (auto j = 0; j < nw.neurals.size(); ++j)
					math::matrix_assign(nwt.neurals[j].weight(), nw.neurals[j].weight(), nw.neurals[j].row(), nw.neurals[j].column());
			}
